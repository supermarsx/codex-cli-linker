    ...
    while True:
        ...
        for i, (lbl, val, dsc) in enumerate(items, 1):
            ...
            if dsc:
                line += " " + c(f"[{dsc}]", GRAY)
            print(line)
        act = prompt_choice("Action", ["Edit field", "Edit all fields", "Save", "Cancel", fmt("🏠 Back to main menu")])
        if not getattr(args, "continuous", False):
            try:
                clear_screen()
            except Exception:
        ...
        if act == 0:
            ...
            idx = int(s) - 1
            if idx == 0:
                m = prompt_choice("Set provider", ["Choose from existing providers", "Enter manually"])
                if m == 0:
                    names: list[str] = []
                ...
                s2 = _safe_input("Max output tokens (blank to skip, 'null' to clear): ").strip()
                if s2:
                ...
                i2 = prompt_choice("Approval policy", ["untrusted", "on-failure", "on-request", "never", "Skip (no change)", "Set to null"])
                if i2 < 4:
                    ov["approval_policy"] = ["untrusted", "on-failure", "on-request", "never"][i2]
                elif i2 == 5:
                    ov["approval_policy"] = ""
            elif idx == 5:
                i2 = prompt_choice("File opener", ["vscode", "vscode-insiders", "windsurf", "cursor", "none", "Skip (no change)", "Set to null"])
                if i2 < 5:
                    ov["file_opener"] = ["vscode", "vscode-insiders", "windsurf", "cursor", "none"][i2]
                elif i2 == 6:
                    ov["file_opener"] = ""
            elif idx == 6:
                ...
                s2 = _safe_input("ChatGPT base URL (blank to skip, 'null' to clear): ").strip()
                if s2:
                    if _is_null_input(s2):
                        ov["chatgpt_base_url"] = ""
                    else:
                ...
                s2 = _safe_input("History max bytes (blank to skip, 'null' to clear): ").strip()
                if s2:
                    if _is_null_input(s2):
